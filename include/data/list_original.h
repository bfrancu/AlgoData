...
protected:
    ...
    node_pointer create(Args&&... args) {
        ...
        try{
            ...
            node_alloc_traits::construct(m_alloc, raw_p, std::forward<Args>(args)...);
        }
        catch(...){
//            std::cout <<"list::create() exception catched\n";
    ...
    {
        auto p_new_node = create(std::forward<U>(val), m_id, p_node, p_node->next);
...
//        puts("list: insertBefore()");
        auto p_new_node = create(std::forward<U>(val), m_id, p_node->prev, p_node);
    ...
    {
        m_header = create(T{}, m_id, nullptr, nullptr);
        m_tail = create(T{}, m_id, m_header, nullptr);
